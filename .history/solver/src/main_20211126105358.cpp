...
void get_pic_points(vector<Point2f> pic_points, Size imageSize, string path, Mat cameraMatrix, Mat distCoeff, int distortion_flag){
    ifstream ifile;
    ifile.open(path);
        if (ifile) {
        string line;
...
int main(int argc, char** argv){
    ...
    get_camera_intrinsic(path, imageSize, cameraMatrix, distCoeff, distortion_flag);

    ifstream ifile;
    string path_pic_points = "../img_data/pic_points.txt";
    ifile.open(path_pic_points);

    vector<Point2f> pic_points;

    get_pic_points(pic_points, imageSize, path, cameraMatrix, distCoeff, distortion_flag);
