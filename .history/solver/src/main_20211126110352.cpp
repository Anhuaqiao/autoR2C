...
void get_camera_intrinsic(string path, Size imageSize, Mat cameraMatrix, Mat distCoeff, int distortion_flag){
    ...
    fs["distortion"] >> distortion_flag;
    fs.release();
}

void get_pic_points(string path, vector<Point2f> pic_points, Size imageSize, string path, Mat cameraMatrix, Mat distCoeff, int distortion_flag){
    ifstream ifile;
    ifile.open(path);
        if (ifile) {
        string line;
...
int main(int argc, char** argv){
    ...
    get_pic_points(path_pic_points,pic_points, imageSize, path_pic_points, cameraMatrix, distCoeff, distortion_flag);
    string path_rad_config = "../radar_data/config.yaml";
    Get_rd rd;
    get_rad_points(path_rad_config, rd);
