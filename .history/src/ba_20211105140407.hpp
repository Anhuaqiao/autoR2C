...
struct ReprojectAdjuster{
    ...
    template<typename T>
    bool operator()(const T* const rvec, const T* const tvec, const T* const residuals) const {
        T p[3];
        T p_[3];
        T p__[3];
        T u, v;
        ...
        p__[1] = p_[1]*(T(1)+k1*r2+k2*r2*r2) + p1*(r2 + 2*p_[1]*p_[1]) + T(2)*p2*p_[0]*p_[1];
    ...
    static ceres::CostFunction *Create(int pixel_u, int pixel_v, double* CameraMatrix, double* DistCoeff) {
        return (new ceres::AutoDiffCostFunction<ReprojectAdjuster, 2, 3, 3, 3>(
                new ReprojectAdjuster(pixel_u, pixel_v, CameraMatrix, DistCoeff)));
    }
