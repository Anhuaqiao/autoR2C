    ...
    ifile.open(path);
        ...
        for (size_t i=0; i<filenames.size(); i++)
        {
            ...
            if (distortion_flag) {
                ...
                Eliminator Eliminate(img_1, cameraMatrix, distCoeff, imageSize,imageWidth, imageHeight);// 消除畸变
            }
            // 点击角反射器中心 图片坐标系坐标
...
void autoR2C::run_calibrate(){
    ...
    verification(path_pic_verify, output_path, path_rad, result_path_ba, rvec_ba, tvec_ba, cameraMatrix, distCoeff, pic_points, rd, imageSize);
}
