...
void autoR2C::get_pic_points(string path, vector<Point2f>& pic_points, Size imageSize,  Mat cameraMatrix, Mat distCoeff, int distortion_flag){
    ...
    if (ifile) {
        cout << " got in here loop" << endl;
        string line;
        while ( getline(ifile, line) ){
            cout << " got in here loop loop" << endl;
            istringstream iss(line);
            int a, b;
            cout << " got in here loop loop loop" << endl;
            if(!( iss >> a >> b )) { break; }
            cout << " got in here loop loop loop loop" << endl;
            cout << "here pic points" << endl;
            cout << a << b << endl;
...
void autoR2C::verification(string path, string output_path, string path_rad, string result_path, Mat rVec, Mat tVec, Mat cameraMatrix, Mat distCoeff, vector<Point2f>& pic_points, Get_rd rd, Size imageSize){
    string path_config = path + "/config.yaml";
...
void autoR2C::run_calibrate(){
    ...
    cout << rVec << endl;
    cout << tVec << endl;
    verification(path_test, output_path, path_rad, result_path, rVec, tVec, cameraMatrix, distCoeff, pic_points, rd, imageSize);
    ba adjuster(result_path);
    SolveBA(adjuster);
    cv::Mat rvec_ba(3, 1, CV_64F, adjuster.RvecInit());
    cv::Mat tvec_ba(3, 1, CV_64F, adjuster.TvecInit());
    verification(path_test, output_path, path_rad, result_path_ba, rvec_ba, tvec_ba, cameraMatrix, distCoeff, pic_points, rd, imageSize);
}
